      ...
      {productData.map(({_id, title, brand, category, description, image, isNew, oldPrice, price}: ProductProps)=>(
        <div key={_id} className='w-full bg-white text-gray-500 rounded-lg group overflow-hidden'>
          ...
          <div className='px-4 py-3 flex flex-col gap-1'>
           ...
           <p className='text-xs text-gray-600 text-justify'>{description.substring(0, 100)}</p>
           <button className='h-10 font-medium bg-amazon_blue text-white rounded-md hover:bg-amazon_yellow hover:text-black duration-300'>Add to Cart</button>
          </div>
        </div>
      ))}
    </div>
